...
let todoBannerDismissedToday = sessionStorage.getItem(TODO_BANNER_DISMISS_KEY) === '' + getTodayDateStr();
...
function loadArchived() {
    try { var raw = localStorage.getItem(ARCHIVED_STORAGE_KEY); return raw ? JSON.parse(raw) : []; }
    catch (e) { return []; }
}
function saveArchived(list) {
    try { localStorage.setItem(ARCHIVED_STORAGE_KEY, JSON.stringify(list)); } catch (e) {}
}

function loadCustomCourses() {
    try { var raw = localStorage.getItem(CUSTOM_STORAGE_KEY); return raw ? JSON.parse(raw) : []; }
    catch (e) { return []; }
}
function saveCustomCourses(courses) {
    try { localStorage.setItem(CUSTOM_STORAGE_KEY, JSON.stringify(courses)); } catch (e) { alert('Failed to save. Storage may be full.'); }
}

function loadCourseEdits() {
    try { return JSON.parse(localStorage.getItem(COURSE_EDIT_KEY)) || {}; } catch (e) { return {}; }
}
function saveCourseEdits(edits) {
    try { localStorage.setItem(COURSE_EDIT_KEY, JSON.stringify(edits)); } catch (e) {}
}

/* --- Study --- */
function loadStudyCompletion() {
    try { return JSON.parse(localStorage.getItem(STUDY_STORAGE_KEY)) || {}; } catch (e) { return {}; }
}
function saveStudyCompletion(map) {
    try { localStorage.setItem(STUDY_STORAGE_KEY, JSON.stringify(map)); } catch (e) {}
}
...
function loadStudyWeek() {
    try { return localStorage.getItem(STUDY_WEEK_KEY) || ''; } catch (e) { return ''; }
}
function saveStudyWeek(val) {
    try { localStorage.setItem(STUDY_WEEK_KEY, val); } catch (e) {}
}

function loadStudyEdits() {
    try { return JSON.parse(localStorage.getItem(STUDY_EDIT_KEY)) || {}; } catch (e) { return {}; }
}
function saveStudyEdits(edits) {
    try { localStorage.setItem(STUDY_EDIT_KEY, JSON.stringify(edits)); } catch (e) {}
}

function loadCustomSubjects() {
    try { return JSON.parse(localStorage.getItem(CUSTOM_SUBJECT_KEY)) || []; } catch (e) { return []; }
}
function saveCustomSubjects(subjects) {
    try { localStorage.setItem(CUSTOM_SUBJECT_KEY, JSON.stringify(subjects)); } catch (e) {}
}
...
/* --- Todo --- */
function loadTodos() {
    try { return JSON.parse(localStorage.getItem(TODO_STORAGE_KEY)) || []; } catch (e) { return []; }
}
function saveTodos(list) {
    try { localStorage.setItem(TODO_STORAGE_KEY, JSON.stringify(list)); } catch (e) {}
}

/* --- Finance --- */
function loadFinanceRecords() {
    try {
        if (!localStorage.getItem(FINANCE_STORAGE_KEY) && localStorage.getItem('Finance_records')) {
            localStorage.setItem(FINANCE_STORAGE_KEY, localStorage.getItem('Finance_records'));
            localStorage.removeItem('Finance_records');
        }
    } catch (e) {}
    try { return JSON.parse(localStorage.getItem(FINANCE_STORAGE_KEY)) || []; } catch (e) { return []; }
}
...
function loadCertificates() {
    try { var raw = localStorage.getItem(CERT_STORAGE_KEY); return raw ? JSON.parse(raw) : []; }
    catch (e) { return []; }
}
function saveCertificates(certs) {
    try { localStorage.setItem(CERT_STORAGE_KEY, JSON.stringify(certs)); } catch (e) { alert('Gagal menyimpan: ukuran gambar terlalu besar untuk localStorage.'); }
}
...
function loadCourseNotes() {
    try { return JSON.parse(localStorage.getItem(COURSE_NOTES_KEY)) || {}; }
    catch (e) { return {}; }
}
function saveCourseNotes(notes) {
    try { localStorage.setItem(COURSE_NOTES_KEY, JSON.stringify(notes)); }
    catch (e) {}
}
...
function getCourseKey(course) {
    ...
    if (typeof course.id === 'string' && course.id.indexOf('custom_') === 0) return course.id;
    return 'c_' + course.id;
}
...
function getCoursesWithCompletion() {
    ...
    var result = COURSES.map(function (course) {
        return Object.assign({}, course, { completed: !!map[getCourseKey(course)] });
    });
...
function applySearch(courses) {
    ...
    return courses.filter(function (course) {
        var haystack = (course.title + ' ' + (course.description || '') + ' ' + (course.company || '') + ' ' + (course.role || '') + ' ' + (course.subCategory || '')).toLowerCase();
...
function excludeArchived(courses) {
    ...
    return courses.filter(function (c) { return archived.indexOf(getCourseKey(c)) === -1; });
}
...
function getFilteredCourses() {
    ...
    if (filterRole) courses = courses.filter(function (c) { return c.role === filterRole; });
    if (filterCategory) courses = courses.filter(function (c) { return c.subCategory === filterCategory; });
    if (filterCompany) courses = courses.filter(function (c) { return c.company === filterCompany; });
    return courses;
}
...
function populateTimeSelect(selectEl) {
    ...
    selectEl.innerHTML = slots.map(function (t) { return '<option value="' + t + '">' + t + '</option>'; }).join('');
    selectEl.dataset.populated = 'true';
}

function parseJamRange(jamStr) {
    if (!jamStr || jamStr === '\u2013' || jamStr === '-') return { mulai: '08.00', selesai: '09.30' };
    var parts = jamStr.split(/[\u2013-]/).map(function (s) { return s.trim(); });
...
function renderMarkdown(src) {
    ...
    function inline(text) {
        ...
        text = text.replace(/(?:^|[^*_])_(.+?)_(?!\w)/g, function (m, p1) { return m.replace('_' + p1 + '_', '<em>' + p1 + '</em>'); });
    ...
    lines.forEach(function (line) {
        ...
        if (headingMatch) {
            ...
            html.push('<h' + (level + 3) + '>' + inline(headingMatch[2]) + '</h' + (level + 3) + '>');
        } else if (listMatch) {
...
function stripMarkdown(src) {
    if (!src) return '';
    return src.replace(/^#{1,6}\s+/gm, '').replace(/\*\*(.+?)\*\*/g, '$1').replace(/\*(.+?)\*/g, '$1').replace(/_(.+?)_/g, '$1').replace(/^[-*]\s+/gm, '\u2022 ').replace(/\[(.+?)\]\(.+?\)/g, '$1').trim();
}

function insertMarkdownSyntax(textarea, type) {
    if (!textarea) return;
    var start = textarea.selectionStart, end = textarea.selectionEnd;
    var value = textarea.value, selected = value.slice(start, end);
    var before = value.slice(0, start), after = value.slice(end);
    var newText = '', cursorOffset = 0;
    switch (type) {
        case 'bold': newText = '**' + (selected || 'teks tebal') + '**'; cursorOffset = selected ? newText.length : 2; break;
        case 'italic': newText = '_' + (selected || 'teks miring') + '_'; cursorOffset = selected ? newText.length : 1; break;
        case 'heading': newText = '### ' + (selected || 'Judul'); cursorOffset = newText.length; break;
        case 'list': newText = (selected ? selected.split('\n').map(function (l) { return '- ' + l; }).join('\n') : '- item'); cursorOffset = newText.length; break;
        case 'link': newText = '[' + (selected || 'teks link') + '](https://)'; cursorOffset = newText.length; break;
        default: return;
    }
    ...
    try {
        ...
        Object.keys(data).forEach(function (feature) {
            Object.keys(data[feature]).forEach(function (key) {
                try { localStorage.setItem(key, JSON.stringify(data[feature][key])); } catch (e) {}
            });
        });
        return true;
    } catch (e) {
...
function createViewDropdown(containerId, options, selectedValue, onChange, isTime) {
    var container = document.getElementById(containerId);
    if (!container) return;
    var panelCls = isTime ? ' view-dropdown__panel--time' : '';
    var selVal = selectedValue || (options[0] || '');
    var html = '<div class="view-dropdown" id="dd-' + containerId + '" style="min-width:100%;">' +
        '<button class="view-dropdown__trigger" type="button"><span class="view-dropdown__label">' + selVal + '</span><i data-lucide="chevron-down"></i></button>' +
        '<div class="view-dropdown__panel' + panelCls + '">';
    options.forEach(function (opt) {
        var sel = opt === selVal ? ' is-selected' : '';
        html += '<button class="view-dropdown__item' + sel + '" data-value="' + opt.replace(/"/g, '&quot;') + '">' + opt + '</button>';
    });
    html += '</div></div>';
...
function getCurrencySymbol(currency) {
    var map = { IDR: 'Rp', USD: '$', EUR: '\u20ac', GBP: '\u00a3', JPY: '\u00a5', KRW: '\u20a9', CNY: '\u00a5', SGD: '$', MYR: 'RM', AUD: '$', CAD: '$', CHF: 'Fr', HKD: '$', THB: '\u0e3f', INR: '\u20b9', PHP: '\u20b1', VND: '\u20ab', SAR: '\ufdfc' };
...
function switchTab(tabName) {
    ...
    document.querySelectorAll('.bottom-nav__item[data-tab]').forEach(function (item) {
        item.classList.toggle('active', item.dataset.tab === tabName);
    });
    updateHeaderCounter(tabName);
    updateTodoDueBadge();
    document.getElementById('panel-courses').classList.toggle('hidden', tabName !== 'courses');
    document.getElementById('panel-analytics').classList.toggle('hidden', tabName !== 'analytics');
    document.getElementById('panel-certificate').classList.toggle('hidden', tabName !== 'certificate');
    document.getElementById('panel-archived').classList.toggle('hidden', tabName !== 'archived');
    document.getElementById('panel-study').classList.toggle('hidden', tabName !== 'study');
    document.getElementById('panel-journal').classList.toggle('hidden', tabName !== 'journal');
    document.getElementById('panel-todo').classList.toggle('hidden', tabName !== 'todo');
    document.getElementById('panel-finance').classList.toggle('hidden', tabName !== 'finance');
    var headerCounter = document.getElementById('header-counter');
    if (headerCounter) headerCounter.classList.toggle('hidden', tabName === 'analytics');
    switch (tabName) {
        case 'analytics': renderAnalytics(); renderStudyAnalytics(); renderTodoAnalytics(); renderFinanceAnalytics(); break;
        case 'certificate': renderCertificateGallery(); break;
        case 'archived': renderArchivedTable(); break;
        case 'study': renderStudy(); break;
        case 'journal': renderStudyLog(); break;
...
function attachTabListeners() {
    ...
    document.querySelectorAll('.bottom-nav__item[data-tab]').forEach(function (item) {
        item.addEventListener('click', function () { switchTab(item.dataset.tab); closeBottomSheet(); });
    });
    document.querySelectorAll('.bottom-sheet__item[data-tab]').forEach(function (item) {
        item.addEventListener('click', function () { switchTab(item.dataset.tab); closeBottomSheet(); });
    });
}
...
function updateHeaderCounter(tabName) {
    var labelLeft = document.querySelector('#header-required-count').previousElementSibling;
    var valueLeft = document.getElementById('header-required-count');
    var labelRight = document.querySelector('#header-total-count').previousElementSibling;
    var valueRight = document.getElementById('header-total-count');
    switch (tabName) {
        case 'courses':
        case 'analytics': {
            var allCourses = getCoursesWithCompletion();
            var completed = allCourses.filter(function (c) { return c.completed; }).length;
        ...
        case 'study': {
            ...
            var studyDone = getAllStudyCourses().filter(function (m) { return !!studyComp['mk_' + m.kode]; }).length;
        ...
        case 'archived': {
            ...
            var archivedCount = allArch.filter(function (c) { return archivedKeys.indexOf(getCourseKey(c)) !== -1; }).length;
        ...
        case 'todo': {
            var todos = loadTodos();
            var todoDone = todos.filter(function (t) { return t.completed; }).length;
        ...
        case 'finance': {
            ...
            var financeTotal = financeRecords.reduce(function (sum, r) { return sum + (r.amount || 0); }, 0);
...
function refreshCompletionUI() {
    ...
    var completed = courses.filter(function (c) { return c.completed; });
    var totalCompleted = completed.length;
    var phase1Done = completed.filter(function (c) { return c.phase === 1; }).length;
    var phase3Done = completed.filter(function (c) { return c.phase === 3; }).length;
    ...
    var phase1Total = courses.filter(function (c) { return c.phase === 1; }).length;
    var phase3Total = courses.filter(function (c) { return c.phase === 3; }).length;
    ...
    if (analyticsPanel && !analyticsPanel.classList.contains('hidden')) renderCharts(courses);
...
function updateProgressBars(courses) {
    ...
    roles.forEach(function (role) {
        var total = courses.filter(function (c) { return c.role === role.field; }).length;
        var done = courses.filter(function (c) { return c.role === role.field && c.completed; }).length;
...
function updateRowCompletionState(key, completed) {
    ...
    if (input) { input.checked = completed; input.setAttribute('aria-checked', completed); }
}
...
function getDueTodos() {
    return loadTodos().filter(function (t) { return !!getTodoDueStatus(t); });
}
...
function updateTodoDueBadge() {
    ...
    if (!badge) return;
    if (!isNotifTodoEnabled()) { badge.classList.add('hidden'); return; }
    var due = getDueTodos();
...
function renderTodoDueBanner() {
    ...
    if (!banner) return;
    if (!isNotifTodoEnabled() || todoBannerDismissedToday) { banner.classList.add('hidden'); banner.innerHTML = ''; return; }
    var due = getDueTodos();
    if (!due.length) { banner.classList.add('hidden'); banner.innerHTML = ''; return; }
    var overdueCount = due.filter(function (t) { return getTodoDueStatus(t) === 'overdue'; }).length;
    ...
    banner.innerHTML = '<div class="todo-due-banner__text"><i data-lucide="alert-triangle"></i><span>' + escapeHtml(message.charAt(0).toUpperCase() + message.slice(1)) + '</span></div><button class="todo-due-banner__close" id="todo-due-banner-close" aria-label="Dismiss" title="Dismiss"><i data-lucide="x"></i></button>';
...
function initProfile() {
    ...
    if (settings.avatar && settings.avatar.indexOf('data:image') === 0) { settings.avatar = null; saveSettings(settings); }
...
function init() {
    ...
    if (sheetProfile) sheetProfile.addEventListener('click', function () {
        closeBottomSheet();
        openSettings();
    });
    ...
    if (addBtn) addBtn.addEventListener('click', function () { openCertModal('add'); });
    ...
    if (dropZone && imageInput) {
        ...
        dropZone.addEventListener('drop', function (e) {
            ...
            if (files && files.length) { imageInput.files = files; imageInput.dispatchEvent(new Event('change')); }
        });
    ...
    if (certImageChange) certImageChange.addEventListener('click', function (e) { e.stopPropagation(); imageInput.click(); });
    var certImageRemove = document.getElementById('cert-image-remove');
    if (certImageRemove) certImageRemove.addEventListener('click', function (e) {
        e.stopPropagation();
        imageInput.value = '';
        var preview = document.getElementById('cert-image-preview');
        preview.src = '';
    ...
    if (certModal) certModal.addEventListener('click', function (e) { if (e.target === certModal) closeCertModal(); });
    ...
    if (addCourseBtn) addCourseBtn.addEventListener('click', function () {
        createViewDropdown('add-course-role-container', ['FullStack Developer', 'Software Engineer', 'AI Engineer', 'Others'], 'Others');
    ...
    if (addCourseCancel) addCourseCancel.addEventListener('click', function () { document.getElementById('add-course-modal').classList.remove('is-open'); });
    var addCourseSave = document.getElementById('add-course-save');
    if (addCourseSave) addCourseSave.addEventListener('click', function () {
        var title = document.getElementById('add-course-title').value.trim();
        var desc = document.getElementById('add-course-desc').value.trim();
        var category = document.getElementById('add-course-category').value.trim();
        ...
        var phase = parseInt(getViewDropdownValue('add-course-phase-container')) || 1;
        if (!title) { showAlert('Please enter a course name.'); return; }
        if (!category) { showAlert('Please enter a category.'); return; }
        if (!company) { showAlert('Please enter a company or platform.'); return; }
        var courses = loadCustomCourses();
        if (courseEditKey) {
            var found = false;
            for (var ci = 0; ci < courses.length; ci++) {
                if (getCourseKey(courses[ci]) === courseEditKey) {
                    courses[ci].title = title; courses[ci].description = desc; courses[ci].subCategory = category;
                    courses[ci].company = company; courses[ci].url = url; courses[ci].role = role; courses[ci].phase = phase; found = true; break;
                }
            }
            if (!found) {
                var edits = loadCourseEdits();
                edits[courseEditKey] = { title: title, description: desc, subCategory: category, company: company, url: url, role: role, phase: phase };
                saveCourseEdits(edits);
            }
            courseEditKey = null;
        } else {
            courses.push({ id: 'custom_' + Date.now(), title: title, description: desc, subCategory: category, company: company, url: url, role: role, phase: phase, completed: false });
        }
        saveCustomCourses(courses);
        ['add-course-title', 'add-course-desc', 'add-course-category', 'add-course-company', 'add-course-url'].forEach(function (id) { document.getElementById(id).value = ''; });
        document.getElementById('add-course-modal').classList.remove('is-open');
        renderTable();
        refreshCompletionUI();
    });
    var addModal = document.getElementById('add-course-modal');
    if (addModal) addModal.addEventListener('click', function (e) { if (e.target === addModal) addModal.classList.remove('is-open'); });
    ...
    if (studyModal) studyModal.addEventListener('click', function (e) { if (e.target === studyModal) closeStudyEntryModal(); });
    var studyImageInput = document.getElementById('study-input-image');
    if (studyImageInput) studyImageInput.addEventListener('change', function () { var file = studyImageInput.files[0]; if (file) handleStudyImageUpload(file); });
    var studyDropZone = document.getElementById('study-drop-zone');
    if (studyDropZone && studyImageInput) {
        ['dragenter', 'dragover'].forEach(function (evt) {
            studyDropZone.addEventListener(evt, function (e) { e.preventDefault(); e.stopPropagation(); studyDropZone.classList.add('drag-over'); });
        });
        ['dragleave', 'drop'].forEach(function (evt) {
            studyDropZone.addEventListener(evt, function (e) { e.preventDefault(); e.stopPropagation(); studyDropZone.classList.remove('drag-over'); });
        });
        studyDropZone.addEventListener('drop', function (e) { var files = e.dataTransfer.files; if (files && files.length) { studyImageInput.files = files; studyImageInput.dispatchEvent(new Event('change')); } });
        studyDropZone.addEventListener('click', function (e) { if (e.target.closest('.file-drop-zone__preview-actions')) return; studyImageInput.click(); });
    }
    var studyImageChange = document.getElementById('study-image-change');
    if (studyImageChange) studyImageChange.addEventListener('click', function (e) { e.stopPropagation(); studyImageInput.click(); });
    var studyImageRemove = document.getElementById('study-image-remove');
    if (studyImageRemove) studyImageRemove.addEventListener('click', function (e) {
        e.stopPropagation(); studyImageInput.value = '';
        var preview = document.getElementById('study-image-preview');
        preview.src = ''; delete preview.dataset.blob;
    ...
    if (mdToolbar) mdToolbar.addEventListener('click', function (e) {
        var btn = e.target.closest('.markdown-toolbar__btn');
        if (!btn) return;
        insertMarkdownSyntax(document.getElementById('study-entry-ringkasan'), btn.dataset.md);
    });

    /* --- Journal detail lightbox --- */
    var journalDetailClose = document.getElementById('journal-detail-close');
    if (journalDetailClose) journalDetailClose.addEventListener('click', closeJournalDetailLightbox);
    var journalDetailModal = document.getElementById('journal-detail-modal');
    if (journalDetailModal) journalDetailModal.addEventListener('click', function (e) { if (e.target === journalDetailModal) closeJournalDetailLightbox(); });
    var journalDetailEdit = document.getElementById('journal-detail-edit');
    if (journalDetailEdit) journalDetailEdit.addEventListener('click', function () {
        var log = loadStudyLog();
    ...
    if (journalDetailDelete) journalDetailDelete.addEventListener('click', function () { requestDeleteJournalEntry(journalDetailEntryId); });
    var journalDeleteCancel = document.getElementById('journal-delete-cancel');
    if (journalDeleteCancel) journalDeleteCancel.addEventListener('click', function () { document.getElementById('journal-delete-modal').classList.remove('is-open'); });
    var journalDeleteConfirmBtn = document.getElementById('journal-delete-confirm');
    if (journalDeleteConfirmBtn) journalDeleteConfirmBtn.addEventListener('click', confirmDeleteJournalEntry);
    var journalDeleteModal = document.getElementById('journal-delete-modal');
    if (journalDeleteModal) journalDeleteModal.addEventListener('click', function (e) { if (e.target === journalDeleteModal) journalDeleteModal.classList.remove('is-open'); });
    ...
    if (studyEditCancel) studyEditCancel.addEventListener('click', closeStudyEditModal);
    var studyEditModal = document.getElementById('study-edit-modal');
    if (studyEditModal) studyEditModal.addEventListener('click', function (e) { if (e.target === studyEditModal) closeStudyEditModal(); });
    ...
    if (studyAddCancel) studyAddCancel.addEventListener('click', closeStudyAddModal);
    var studyAddModal = document.getElementById('study-add-modal');
    if (studyAddModal) studyAddModal.addEventListener('click', function (e) { if (e.target === studyAddModal) closeStudyAddModal(); });

    /* --- To-do tab --- */
    var todoAddBtn = document.getElementById('todo-add-btn');
    if (todoAddBtn) todoAddBtn.addEventListener('click', function () { openTodoModal('add'); });
    ...
    if (todoModal) todoModal.addEventListener('click', function (e) { if (e.target === todoModal) closeTodoModal(); });
    var todoDelConfirm = document.getElementById('todo-delete-confirm');
    if (todoDelConfirm) todoDelConfirm.addEventListener('click', function () {
        var id = todoDelConfirm.dataset.targetId;
        if (id) { var list = loadTodos(); list = list.filter(function (t) { return t.id !== id; }); saveTodos(list); renderTodos(); }
    ...
    if (todoDelCancel) todoDelCancel.addEventListener('click', function () { document.getElementById('todo-delete-modal').classList.remove('is-open'); });
    var todoDelModal = document.getElementById('todo-delete-modal');
    if (todoDelModal) todoDelModal.addEventListener('click', function (e) { if (e.target === todoDelModal) todoDelModal.classList.remove('is-open'); });
    var todoPrioGroup = document.getElementById('todo-priority-group');
    if (todoPrioGroup) todoPrioGroup.addEventListener('click', function (e) {
        var btn = e.target.closest('.btn-group__item');
        if (!btn) return;
        todoPrioGroup.querySelectorAll('.btn-group__item').forEach(function (b) { b.classList.remove('btn-group__item--active'); });
    ...
    if (FinanceCurrDD) {
        ...
        if (currTrigger) currTrigger.addEventListener('click', function (e) { e.stopPropagation(); FinanceCurrDD.classList.toggle('is-open'); });
        FinanceCurrDD.querySelectorAll('.view-dropdown__item').forEach(function (item) {
            item.addEventListener('click', function (e) {
                e.stopPropagation();
                FinanceCurrDD.querySelectorAll('.view-dropdown__item').forEach(function (i) { i.classList.remove('is-selected'); });
                item.classList.add('is-selected');
                if (currLabel) currLabel.textContent = item.textContent.trim();
                document.getElementById('finance-input-currency').value = item.dataset.value;
                FinanceCurrDD.classList.remove('is-open');
            });
        });
        document.addEventListener('click', function () { FinanceCurrDD.classList.remove('is-open'); });
    }
    ...
    if (FinanceWeekSearch) {
        ...
        FinanceWeekSearch.addEventListener('input', function () {
            ...
            FinanceSearchTimer = setTimeout(function () {
                ...
                if (match) { var wn = parseInt(match[1]); updateFinanceWeekLabel(wn); renderFinanceWeek(wn); }
                else { var cw = getWeekNumber(new Date()); updateFinanceWeekLabel(cw); renderFinanceWeek(cw); }
            }, 300);
        });
    }
    ...
    if (FinanceModalSave) FinanceModalSave.addEventListener('click', saveFinanceFromModal);
    ...
    if (FinanceModal) FinanceModal.addEventListener('click', function (e) { if (e.target === FinanceModal) closeFinanceModal(); });
    var FinanceDelConfirm = document.getElementById('finance-delete-confirm');
    if (FinanceDelConfirm) FinanceDelConfirm.addEventListener('click', function () {
        var id = FinanceDelConfirm.dataset.targetId;
        if (id) { var list = loadFinanceRecords(); list = list.filter(function (r) { return r.id !== id; }); saveFinanceRecords(list); renderFinance(); }
        document.getElementById('finance-delete-modal').classList.remove('is-open');
    });
    var FinanceDelCancel = document.getElementById('finance-delete-cancel');
    if (FinanceDelCancel) FinanceDelCancel.addEventListener('click', function () { document.getElementById('finance-delete-modal').classList.remove('is-open'); });
    var FinanceDelModal = document.getElementById('finance-delete-modal');
    if (FinanceDelModal) FinanceDelModal.addEventListener('click', function (e) { if (e.target === FinanceDelModal) FinanceDelModal.classList.remove('is-open'); });
    var FinanceExportBtn = document.getElementById('finance-export-btn');
    if (FinanceExportBtn) FinanceExportBtn.addEventListener('click', function () {
        var label = document.getElementById('finance-week-label');
        var weekLabel = label ? label.textContent : 'Minggu ' + getWeekNumber(new Date());
        document.getElementById('finance-export-week-label').textContent = weekLabel;
    ...
    if (FinanceExportCsv) FinanceExportCsv.addEventListener('click', function () {
        var label = document.getElementById('finance-week-label');
        var week = label ? parseInt(label.textContent.replace('Minggu ', '')) : getWeekNumber(new Date());
        exportFinanceCSV(week);
        document.getElementById('finance-export-modal').classList.remove('is-open');
    });
    var FinanceExportPdf = document.getElementById('finance-export-modal-pdf');
    if (FinanceExportPdf) FinanceExportPdf.addEventListener('click', function () {
        var label = document.getElementById('finance-week-label');
        var week = label ? parseInt(label.textContent.replace('Minggu ', '')) : getWeekNumber(new Date());
        exportFinancePDF(week);
        document.getElementById('finance-export-modal').classList.remove('is-open');
    });
    var FinanceExportClose = document.getElementById('finance-export-modal-close');
    if (FinanceExportClose) FinanceExportClose.addEventListener('click', function () { document.getElementById('finance-export-modal').classList.remove('is-open'); });
    var FinanceExportModal = document.getElementById('finance-export-modal');
    if (FinanceExportModal) FinanceExportModal.addEventListener('click', function (e) { if (e.target === FinanceExportModal) FinanceExportModal.classList.remove('is-open'); });
    ...
    document.addEventListener('keydown', function (e) {
        if (e.key === 'Escape') {
            ...
            if (actionModal && actionModal.classList.contains('is-open')) closeAvatarSubModal();
            else if (settingsModal && settingsModal.classList.contains('is-open')) closeSettings();
        }
    });
    document.querySelectorAll('.settings-nav__item').forEach(function (item) {
        item.addEventListener('click', function () { switchSettingsSection(item.dataset.section); });
    });
    var avatarOverlay = document.getElementById('settings-avatar-overlay');
    if (avatarOverlay) avatarOverlay.addEventListener('click', function (e) { e.stopPropagation(); openAvatarSubModal(); });
    var avatarActionModal = document.getElementById('avatar-action-modal');
    if (avatarActionModal) avatarActionModal.addEventListener('click', function (e) { if (e.target === avatarActionModal) closeAvatarSubModal(); });
    ...
    if (avatarActionUpload && avatarInput) {
        avatarActionUpload.addEventListener('click', function () { closeAvatarSubModal(); avatarInput.click(); });
        avatarInput.addEventListener('change', function () { if (avatarInput.files[0]) handleSettingsAvatarUpload(avatarInput.files[0]); });
    }
    var avatarActionDelete = document.getElementById('avatar-action-delete');
    if (avatarActionDelete) avatarActionDelete.addEventListener('click', function () { deleteAvatarPhoto(); });
    var settingsThemeGroup = document.getElementById('settings-theme-group');
    if (settingsThemeGroup) settingsThemeGroup.addEventListener('click', function (e) {
        var btn = e.target.closest('.btn-group__item');
        if (!btn) return;
        settingsThemeGroup.querySelectorAll('.btn-group__item').forEach(function (b) { b.classList.remove('btn-group__item--active'); });
        btn.classList.add('btn-group__item--active');
    ...
    if (settingsLangGroup) settingsLangGroup.addEventListener('click', function (e) {
        var btn = e.target.closest('.btn-group__item');
        if (!btn) return;
        settingsLangGroup.querySelectorAll('.btn-group__item').forEach(function (b) { b.classList.remove('btn-group__item--active'); });
        btn.classList.add('btn-group__item--active');
    ...
    if (exportBtn && exportWrap) exportBtn.addEventListener('click', function (e) { e.stopPropagation(); exportWrap.classList.toggle('is-open'); });
    if (exportMenu) exportMenu.querySelectorAll('.export-dropdown-menu__item').forEach(function (item) {
        item.addEventListener('click', function (e) { e.stopPropagation(); exportScopedData(item.dataset.export); exportWrap.classList.remove('is-open'); });
    });
    document.addEventListener('click', function () { if (exportWrap) exportWrap.classList.remove('is-open'); });
    ...
    if (importBtn && importInput) {
        ...
        importInput.addEventListener('change', function () { if (importInput.files[0]) importAllData(importInput.files[0]); });
    }
    ...
    if (courseDelConfirm) courseDelConfirm.addEventListener('click', function () {
        if (pendingDeleteKey) { deleteCourseByKey(pendingDeleteKey); pendingDeleteKey = null; }
        if (studyPendingDeleteKey) { deleteStudySubject(studyPendingDeleteKey); studyPendingDeleteKey = ''; }
        courseDelModal.classList.remove('is-open');
    });
    if (courseDelCancel) courseDelCancel.addEventListener('click', function () { pendingDeleteKey = null; studyPendingDeleteKey = ''; courseDelModal.classList.remove('is-open'); });
    if (courseDelModal) courseDelModal.addEventListener('click', function (e) {
        if (e.target === courseDelModal) { pendingDeleteKey = null; studyPendingDeleteKey = ''; courseDelModal.classList.remove('is-open'); }
    });
}
